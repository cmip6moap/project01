...
from settings_v2 import targetperiods, baseline_period, datafolder, outputfolder, eflag
...
## SKIP if steric timeseries is full of NaN
        if np.isnan(z).all():
            ...
            continue
    ## Read TAS data
        tas = tas_tools_v2.load_tas(model = n.model, scenario = scenario, run = n.run)
    ...
    ## we will calculate historical baseline here if the file exits
        if not base_key in Tbaselines:
            tas_his = tas_tools_v2.load_tas(model = n.model, scenario = "historical", run = n.run)
...
fout = f'{outputfolder}/StericTvsRate.csv'
output.to_csv(fout)
